...
The 3D files are in [STL](https://en.wikipedia.org/wiki/STL_%28file_format%29) format, which is widely used in 3D printing all over the world. The software for your 3D printer will have no problem loading them, but you may need to change its import settings to use metric millimetres (mm) as opposed to inches. No further scaling should be required.
